...
public final class ICloud {
	 ...
	 * @since 1.1
	 */
	public static UserData queryData(final String server, final String username, final String password)
			throws ICloudException {
		...
		try {
			...
			throw new ICloudException("Error: coulnd't fetch requested data.", e);
		}
	 ...
	 * @since 1.1
	 */
	private ICloud(final String server, final String username, final String password) throws RequestException {
		this.server = server;
	 ...
	 * @since 1.1
	 */
	private UserData getUserData() throws RequestException, FactoryException, ServerException, XMLException {
		final HttpClient client = factory.getHttpClient(server, username, password);
	 ...
	 * @since 1.1
	 */
	private List<Calendar> getCalendars(final HttpClient client, final String principal) throws RequestException,
			FactoryException, ServerException, XMLException {
		final String request = requestMaker.makeRequest("displayname");
		final PropFindMethod propfind = factory.getPropfind("https://" + server + ":443/" + principal + "/calendars/",
				request);
		...
		try {
			...
			if (propfind.getStatusCode() != 207) {
				throw new ServerException("Error: unable to authenticate.", null);
			}

			response = URLDecoder.decode(propfind.getResponseBodyAsString(), propfind.getResponseCharSet());
	 ...
	 * @since 1.1
	 */
	private String getPrincipal(final HttpClient client) throws RequestException, FactoryException, ServerException,
			XMLException {
		final String request = requestMaker.makeRequest("current-user-principal");
		final PropFindMethod propfind = factory.getPropfind("https://" + server + ":443/", request);
