...
public class CmdLine {
	...
	public static void main(final String[] args) {
		...
		if (args.length != 2) {
			...
			try {
				printCalendarInformation(ICloud.queryData(server,
	...
	private static void printHead() {
		System.out
				.println("iCloud calendar URL fetcher\n"
						...
						+ "Copyright (C) 2012 by NiftySide - Daniel Muehlbachler (http://www.niftyside.com)\n\n");
	}
	...
	private static void printCalendarInformation(final UserData queryData) {
		...
		final String format = "%-" + (lengths.get("name") + 3) + "s %-"
				+ (lengths.get("href") + 3) + "s %-" + (lengths.get("url") + 3)
				+ "s\n";

		System.out.printf(format, "NAME", "HREF", "URL");
		for (final Calendar calendar : queryData.getCalendars()) {
			System.out.printf(format, calendar.getName(), calendar.getHref(),
					calendar.getURL());
		}
